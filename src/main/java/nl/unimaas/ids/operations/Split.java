    	...
    	if(uriExpansion != null && uriExpansion.equals("infer")) {	
    		...
    		if(!registeryFile.exists()) {
    			...
    			FileUtils.copyURLToFile(  
      				  new URL("http://prefix.cc/context"), 
            ...
            System.out.println("Regsitery build finished, total items: "+regCount);
	    	...
	    	for (String splitFragment: splitFragments) {          	
	    		...
	    		if(uriExpansion != null) {
	    			...
	    			if(!uriExpansion.equals("infer")) {
		    			...
		    			bulkUpdate.add(subjectIri, predicateIri, f.createIRI(splitFragment), graphIri);
		    			...
		    			if(splitFragment.indexOf("(") != -1) {
		    			 ...
		    			 //splitFragment = splitFragment.replaceAll("\\[.*?\\]", "").trim();
		    			 //splitFragment = splitFragment.replaceAll("\\\\([^()]*\\\\)", "").trim();
		    			 //splitFragment = splitFragment.replaceAll("\\(.*?\\)", "").trim();
		    			 //splitFragment = splitFragment.replaceAll("[()?;{}]+", " ").trim();
		    			 ...
		    			 if(splitFragment.contains(":")) {
		    				 ...
		    				 String prefix = splitFragment.substring(0, p).toLowerCase().replace(" ", "").trim();
		    				 ...
		    				 if (registery.containsKey(prefix)) {
    	    					 ...
    	    					 predicateIri = f.createIRI(propertyToSplit.substring(0,propertyToSplit.lastIndexOf("/")+1)+"x-"+prefix);
    	    		    			
    	    					 bulkUpdate.add(subjectIri, predicateIri, f.createIRI(splitFragment), graphIri);

		    				 }else {
		    					 
	  					    	 predicateIri = f.createIRI(propertyToSplit.substring(0,propertyToSplit.lastIndexOf("/")+1)+"x-ref");
	  					    	 bulkUpdate.add(subjectIri, predicateIri, f.createLiteral(splitFragment), graphIri);
		    					 
//		    					 for (Entry<String, String[]> e : registery.entrySet()) {
//	    						    if (splitFragment.contains(e.getKey())) {
//	    						         				
//	    						    	splitFragment = registery.get(e.getKey())[2] + id;
//	    		    					 
//	    						    	predicateIri = f.createIRI(propertyToSplit.substring(0,propertyToSplit.lastIndexOf("/")+1)+"x-"+prefix);
//	    		    		    			
//	    						    	bulkUpdate.add(subjectIri, predicateIri, f.createLiteral(splitFragment), graphIri);
